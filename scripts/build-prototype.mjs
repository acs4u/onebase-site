...
const customers = [
  { seg: 'Pro sports & performance', names: ['Las Vegas Raiders', 'Portland Timbers', 'TB12', 'Exos', 'Hong Kong Sports Institute', 'Judo Association of Hong Kong', 'Sports Performance Physical Therapy', 'Novum Performance & Longevity', 'SPORTFIVE'] },
  { seg: 'Fitness clubs', names: ['The Edge Fitness Clubs', 'VIDA Fitness', 'Harbor Square Athletic Club', 'Razor Sharp Fitness', 'Balance Gym', 'PURE Family Fitness', 'Wellness Solutions Inc.'] },
  { seg: 'Recovery & wellness studios', names: ['The Covery', '10X Longevity', 'Collagen Lab', 'Hume', 'Hype Wellness Studio', 'Prairie Health & Wellness', 'SB Wellness Group', 'The Rising Zone', 'Hopson Health Wellness Center', 'Variant Training Lab', 'V2 Wellness Group'] },
  { seg: 'Clinics & medical', names: ['HealthFit', 'Hyperbaric Associates of America', 'Belo Medical Group', 'Inspire Chiropractic & Wellness', 'Revital Health', 'Infinity IV & Wellness', 'North Shore Hyperbarics', 'Restore Sports Medicine', 'Doylestown Sports Medicine Center', 'Penrose Physical Therapy', 'Fick PT & Performance', 'Life Clinics', 'Genesis Surgery', 'Makena Health Maui', 'Dr. Michael Ruscio, DC', 'Jason Alexander Med Spa', 'Elixir'] },
  { seg: 'Brands, athletes & hospitality', names: ['Jake Paul', 'Kayla Barnes', 'Time to Bloom', 'Bang Salon', 'The Fox Tan'] },
];
...
<section style="border-bottom:1px solid var(--line)"><div class="wrap logos" style="padding-block:22px"><span class="eyebrow" style="margin-right:8px">Installed at</span>\${['nerevolution','lafc','healthfit','10x','covery','hype','hume','infinity','revital'].map(l=>D.img['logo-'+l]?\`<img src="\${D.img['logo-'+l]}" alt="\${l}" class="\${['nerevolution','lafc'].includes(l)?'crest':''}">\`:'').join('')}</div></section>
...
<section class="sec" style="border-top:1px solid var(--line)"><div class="wrap stack" style="gap:28px"><div class="row" style="justify-content:space-between;align-items:flex-end"><div class="stack" style="max-width:680px;gap:10px">\${eyebrow('Trusted by operators in 12+ countries')}<h2>From pro teams to neighbourhood studios.</h2></div><a href="#/partners" class="small" style="font-weight:500;text-decoration:underline;text-underline-offset:4px">All customers</a></div><div class="wall">\${D.customers.slice(0,4).map(c=>\`<div><p class="eyebrow" style="margin-bottom:10px">\${c.seg}</p><div class="names">\${c.names.slice(0,6).map(n=>\`<span>\${esc(n)}</span>\`).join('')}</div></div>\`).join('')}</div></div></section>
...
<section class="sec" style="background:var(--surf)"><div class="wrap stack" style="gap:28px">\${eyebrow('Customers')}<h2 style="max-width:680px">Installed in gyms, clinics, studios and team facilities across North America, Australia and Asia.</h2><div class="wall">\${D.customers.map(c=>\`<div><p class="eyebrow" style="margin-bottom:10px">\${c.seg}</p><div class="names">\${c.names.map(n=>\`<span>\${esc(n)}</span>\`).join('')}</div></div>\`).join('')}</div></div></section>
<section class="sec"><div class="wrap">\${eyebrow('Partner logos')}<div class="logos" style="margin-top:16px">\${['nerevolution','lafc','healthfit','10x','covery','hype','hume','infinity','revital'].map(l=>D.img['logo-'+l]?\`<img src="\${D.img['logo-'+l]}" alt="\${l}" class="\${['nerevolution','lafc'].includes(l)?'crest':''}">\`:'').join('')}</div></div></section>
